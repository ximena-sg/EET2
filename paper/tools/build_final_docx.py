...
FUTURE = "Planned extensions follow five lines. First, fine physical resolution of trunk links: ingesting and correlating MAC address tables to assign, with absolute precision, the physical port number to links currently deduced at the VLAN level. Second, structured-cabling registration: a versioned scheme to document static fiber and copper runs that carry no software-deducible frames. Third, remote configuration restoration and provisioning from the web interface, strictly conditioned on the prior incorporation of an authentication and role-based access control (RBAC) layer for the web API. Fourth, health supervision and telemetry (Phase 3): availability and link-state monitoring in near real time through lightweight ICMP/SNMP probes [20], keeping the platform's zero-overhead philosophy. Fifth, closing the empirical gaps identified in Section 7.1: running the included extraction script against the deployment's GitStore to characterize collection timings with real sample sizes and dispersion statistics, assembling a ground-truth topology map to compute precision and recall for each evidentiary tier of the inference engine, and executing a quantitative, timed comparison against Oxidized restricted to the modern segment of the network."

ACK = "This work was developed within the framework of the Network Automation Project III at the Research and Development Center in Computer Science (CIDI), National University of San Martín (UNSAM). The authors thank the host institution's network administration staff for their cooperation and for providing access to the infrastructure throughout the pilot deployment."
