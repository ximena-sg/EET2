...
First, statistical rigor of the operational metrics (Table 3): the reported ranges were read directly from the platform's execution logs during a single institutional deployment, without recording the sample size (number of devices per family, number of collection cycles) or computing dispersion statistics. A rigorous version of this evaluation would log the start and end timestamp of every collection cycle, report *n* per device family together with mean ± standard deviation — or, given the expected skew toward slower legacy devices, median and interquartile range — instead of an observed range, and would ideally repeat the full-network cycle across multiple days to capture operational variability. Because every successful collection is already committed to Git (Section 3), the commit history of the deployment's GitStore is itself a candidate raw dataset for this analysis and is retained for a future controlled study.
